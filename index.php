  ...
  <div class="code-wrap">
    <div class="code-top">
      <div class="code-label">
        ...
        ENKREPSİYON BYPASS ÖRNEĞİ — PYTHON
      </div>
      <div class="dots">
    ...
    <div class="code-body"><pre><code><span class="k">import</span> requests
<span class="k">import</span> re
<span class="k">import</span> binascii
<span class="k">from</span> Crypto.Cipher <span class="k">import</span> AES

<span class="k">def</span> <span class="f">istek</span>(url):
    session = requests.<span class="f">Session</span>()
    session.headers.<span class="f">update</span>({
        <span class="s">"User-Agent"</span>: <span class="s">"Mozilla/5.0 (Windows NT 10.0; Win64; x64)"</span>,
        <span class="s">"Accept"</span>: <span class="s">"*/*"</span>
    })
    r = session.<span class="f">get</span>(url, timeout=<span class="n">30</span>)

    if <span class="s">"slowAES.decrypt"</span> <span class="k">in</span> r.text:
        a = re.<span class="f">search</span>(<span class="s">r'a=toNumbers\("([a-f0-9]+)"\)'</span>, r.text).<span class="f">group</span>(<span class="n">1</span>)
        b = re.<span class="f">search</span>(<span class="s">r'b=toNumbers\("([a-f0-9]+)"\)'</span>, r.text).<span class="f">group</span>(<span class="n">1</span>)
        c = re.<span class="f">search</span>(<span class="s">r'c=toNumbers\("([a-f0-9]+)"\)'</span>, r.text).<span class="f">group</span>(<span class="n">1</span>)

        key       = binascii.<span class="f">unhexlify</span>(a)
        iv        = binascii.<span class="f">unhexlify</span>(b)
        ct        = binascii.<span class="f">unhexlify</span>(c)
        cipher    = AES.<span class="f">new</span>(key, AES.MODE_CBC, iv)
        decrypted = cipher.<span class="f">decrypt</span>(ct)
        cookie    = binascii.<span class="f">hexlify</span>(decrypted).<span class="f">decode</span>()
        session.cookies.<span class="f">set</span>(<span class="s">"__test"</span>, cookie)
        r = session.<span class="f">get</span>(url, timeout=<span class="n">30</span>)

    return r.text

print(istek(<span class="s">"https://örnek.com/api"</span>))</code></pre></div>
  </div>
</main>
